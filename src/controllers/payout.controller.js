...
const submitData = asynchandler(async (req,res)=>{
    const {issueDate,advNo,phoneNo,vehicleNo,insComp,ncb,policyNo,idvVal,premium,reward} =req.body;
    if(!(issueDate||advisoryId||phoneNo||vehicleNo||insComp||ncb||policyNo||idvVal||premium||reward)){
...
const exportData = asynchandler(async(req,res)=>{
    ...
    data.map((datablock)=>{
        for(let key in datablock.advisoryDetails){
            if(key!="__v"||key!="updatedAt"||key!="createdAt"){datablock[key]=datablock.advisoryDetails[key];}
            else{delete datablock.advisoryDetails[key]}
        }
        delete datablock.advisoryDetails;
        for(let key in datablock.userDetails){
            if(key!="__v"||key!="updatedAt"||key!="createdAt"){datablock[key]=datablock.userDetails[key];}
            else{delete datablock.userDetails[key]}
        }
        delete datablock.userDetails;
        for(let key in datablock.vehicleDetails){
            if(key!="__v"||key!="updatedAt"||key!="createdAt"){datablock[key]=datablock.vehicleDetails[key];}
            else{delete datablock.vehicleDetails[key]}
        }
...
const getNum = asynchandler(async(req,res)=>{
    ...
    return res.status(200).json(new apiResponse(200,data[0],"no of policies this month fetched"))
});
